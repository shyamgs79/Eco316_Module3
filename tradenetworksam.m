...
%iteration
good1=randi([1,10],nagents,1);
good2=randi([1,10],nagents,1);

alpha_matrix = rand(nagents, 1);
alpha_matrix(:, 2) = 1-alpha_matrix(:, 1);

for i = 1:nagents
    rprice(i, 1)=((alpha_matrix(i, 1))./(alpha_matrix(i, 2)))*((good2(i, 1))./(good1(i, 1)));
    Matsimprice(i, 1)= rprice(i, 1);
end


%rprice=(alpha./(1-alpha)).*(good2./good1);

rpriceacrosssims=[rpriceacrosssims, rprice];
...
for sim=1:numsims
    ...
    for rounds = 1:100
        ...
        if rounds>1
            ...
            good1=[good1, good1(:,rounds-1)];
            good2=[good2, good2(:,rounds-1)];
            
            
            for i=1:nagents
                %reservation price (always price of G1)
                rprice(i, rounds)=((alpha_matrix(i, 1))./(alpha_matrix(i, 2)))*((good2(i, rounds))./(good1(i, rounds)));
                
            end
        end
        %storing how prices change across rounds withing each simulation
        %         Matsimprice(i, rounds)=rprice(i, rounds);
        ...
        for col=1:((nagents/ngroups)+endagentsoverlap)
            ...
            for row=randperm(ngroups)
                ...
                for j=1:nagents
                    ...
                    poteqprice=(alpha_matrix(i,1)*good2(i, rounds)+alpha_matrix(j,1)*good2(j, rounds))/(alpha_matrix(i,2)*good1(i, rounds)+alpha_matrix(j,2)*good1(j, rounds));
                    
                    %if agent i's reservation price is less than agent j's
                    %reservation price, then agent i will buy g2 (ibuyg2=1)
                    %else agent sell g2 (ibuyg2=-1)
                    if rprice(i, rounds)<=rprice(j, rounds)
                        ibuysg2=1;
                    else
                        ibuysg2=-1;
                    end
                    ...
                    %price
                    potui(j, rounds)=(good2(i, rounds)+poteqprice*ibuysg2)*(good1(i, rounds)-ibuysg2);
                ...
                %one unit.
                [~,chosenj]=max(potui(:, rounds));
                
                %if agent i's reservation price is less that agent chosenj's
                %reservation price, then agent i will buy g2 (ibuyg2=1)
                %else agent sell g2 (ibuyg2=-1)
                if rprice(i, rounds)<=rprice(chosenj, rounds)
                    ibuysg2=1;
                else
                    ibuysg2=-1;
                end
                ...
                eqprice=(alpha_matrix(i,1)*good2(i, rounds)+alpha_matrix(chosenj,1)*good2(chosenj, rounds))/(alpha_matrix(i,2)*good1(i, rounds)+alpha_matrix(chosenj,2)*good1(chosenj, rounds));
                ...
                while trytotrade==1
                    ...
                    uinew=(((good2(i, rounds)+eqprice*ibuysg2))^alpha_matrix(i, 2))*(((good1(i, rounds)-ibuysg2))^alpha_matrix(i, 1));
                    
                    %utility for chosenj from trading one unit
                    ujnew=(((good2(chosenj, rounds)-eqprice*ibuysg2))^alpha_matrix(chosenj, 2))*(((good1(chosenj, rounds)+ibuysg2))^alpha_matrix(chosenj, 1));
                    
                    %current utilities
                    uiold=((good1(i, rounds))^alpha_matrix(i, 1))*((good2(i, rounds))^alpha_matrix(i, 2));
                    ujold=((good1(chosenj, rounds))^alpha_matrix(chosenj, 1))*((good2(chosenj, rounds))^alpha_matrix(chosenj, 2));
                    
                    %trade only if new utilities exceed current utilities
                    %for both agents
                    if uinew>uiold && ujnew>ujold
                        %update goods and reservation price
                        good2(i, rounds)=good2(i, rounds)+eqprice*ibuysg2;
                        good2(chosenj, rounds)=good2(chosenj, rounds)-eqprice*ibuysg2;
                        good1(i, rounds)=good1(i, rounds)-ibuysg2;
                        good1(chosenj, rounds)=good1(chosenj, rounds)+ibuysg2;
%                         rprice(i, rounds+1)=((alpha_matrix(i, 1))./(alpha_matrix(i, 2)))*((good2(i, rounds))./(good1(i, rounds)));
%                         rprice(chosenj, rounds+1)=((alpha_matrix(i, 1))./(alpha_matrix(i, 2)))*((good2(chosenj, rounds))./(good1(chosenj, rounds)));
        ...
        %         rprice(i, sim+1)=good2(i, sim+1)./good1(i, sim+1);
        %         for i = 1:nagents
        %             Matsimprice=[Matsimprice,rprice(i, rounds)];
        %         end
        
        %Matsimprice1=[Matsimprice(i, sim+1),rprice(i, sim+1)];
...
%     axis([-inf, inf, 0.5, 2]);
    ...
    storedgood1=[storedgood1, good1(:,rounds)];
    storedgood2=[storedgood2, good2(:,rounds)];
    
    good1=good1(:,rounds);
    good2=good2(:,rounds);
    good1=0.9*good1 + 0.1*good2;
    good2=0.9*good2 + betarnd(2,2,nagents,1)*10;
